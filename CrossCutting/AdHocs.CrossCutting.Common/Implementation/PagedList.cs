...
namespace CrossCutting.Extention
{
    ...
    public class PagedList<T, TKey> : BasePagedList<T>
    {
            ...
            : base(pageNumber, pageSize, superset?.Count() ?? 0)
        {
            ...
            if (TotalItemCount > 0) InitSubset(superset, keySelector.Compile(), pageNumber, pageSize);
        }

        public PagedList(IFindFluent<T, T> superset, int pageNumber, int pageSize)
            : base(pageNumber, pageSize, (int) superset?.CountDocuments())
        {
            // add items to internal list
            if (TotalItemCount > 0) InitSubset(superset, pageNumber, pageSize);
        }

        public PagedList(IQueryable<T> superset, Func<T, TKey> keySelectorMethod, int pageNumber, int pageSize)
            : base(pageNumber, pageSize, superset?.Count() ?? 0)
        {
            if (TotalItemCount > 0) InitSubset(superset, keySelectorMethod, pageNumber, pageSize);
        }

        public PagedList(IFindFluent<T, T> superset, Func<T, TKey> keySelectorMethod, int pageNumber, int pageSize)
            : base(pageNumber, pageSize, (int) superset?.CountDocuments())
        {
            if (TotalItemCount > 0) InitSubset(superset, pageNumber, pageSize);
        }
        ...
        private void InitSubset(IFindFluent<T, T> superset, int pageNumber, int pageSize)
        {
            // add items to internal list

            var items = pageNumber == 1
                ? superset.Limit(pageSize).ToList()
                : superset.Skip((pageNumber - 1) * pageSize).Limit(pageSize).ToList();
